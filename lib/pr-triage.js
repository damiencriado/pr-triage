class PRTriage {
  ...
  async _getRequestedNumberOfReviews() {
    ...
    return this.github.pulls
      .listRequestedReviewers({ owner, repo, pull_number })
  ...
  async _ensurePRTriageLabelExists() {
    const labelKeys = Object.keys(this._getConfigObjsForLabels());
  ...
  _getCurrentLabelKey() {
    return this.pullRequest.labels
      .map((label) => {
        const filteredConfig = this._getConfigObjsForLabels();
